...
void ofApp::setup() {
    ...
    movies[0].load("card2_ura.mov");
    movies[1].load("card3_ura.mov");
    movies[2].load("Rdoragon_B.mp4");
    movies[3].load("eastfire.mp4");
...
void ofApp::draw(){
    ...
    if(nowStatus == 0){
        for(int i = 0; i < 2; ++i) {
            if(i == 0) movies[i].getTexturePtr()->draw(windowWidth / 2 + 280 * (-1 + i), 0, 260, 455);
            else movies[i].getTexturePtr()->draw(windowWidth / 2 + 260 * (-1 + i), 15, 240, 420);
        }
    }
    else if(nowStatus == 1){
